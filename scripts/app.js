...
var AccumulatorFixedStepSimulationRun, FixedStepSimulationRun, SemiFixedStepSimulationRun, SimSpec, SimulationRun, VariableStepSimulationRun, addSim, createSimFromSpec, render, renderSimList, simSpecs, sims, start, _ref,
...
SimulationRun = (function() {
  ...
  SimulationRun.prototype.progress = function() {
    var disruption, dt, endDisruption, maxSecsDisruption, minSecsDisruption, range, sleep, _results;
...
createSimFromSpec = function(spec) {
  ...
  switch (spec.stepName) {
    case "fixed":
      return new FixedStepSimulationRun(initialState, integrationType, accelerant, 0.02);
    case "variable":
      return new VariableStepSimulationRun(initialState, integrationType, accelerant);
    case "semiFixed":
      return new SemiFixedStepSimulationRun(initialState, integrationType, accelerant, 0.02);
    case "accumulatorFixed":
      return new AccumulatorFixedStepSimulationRun(initialState, integrationType, accelerant, 0.02);
  }
};
...
renderSimList = function() {
  ...
  for (index = _i = 0, _len = simSpecs.length; _i < _len; index = ++_i) {
    ...
    row = $("<tr></tr>").append($("<td>" + spec.initialPosition + "</td>")).append($("<td>" + spec.initialVelocity + "</td>")).append($("<td>" + spec.integrationName + "</td>")).append($("<td>" + acceleration + "</td>")).append($("<td>" + spec.stepName + "</td>")).append($('<td><a href="#" class="remove">Remove</a></td>')).data("specIndex", index).appendTo(simTableBody);
    sim = sims[index];
...
render = function() {
  var canvas, context, height, highestPostionY, index, pixelHeight, pixelWidth, pixelsPerMetre, sim, someRunning, width, x, y, _i, _len;
